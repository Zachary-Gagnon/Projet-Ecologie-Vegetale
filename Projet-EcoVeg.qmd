...
# On suppose que `resultats_significatifs` contient une colonne `Espece`
# et que ton jeu de données s’appelle `bdd`

# On boucle sur les espèces significatives
for (espece in resultats_significatifs$Espece) {

  espece_label <- espece  # nom pour affichage
  espece_col <- paste0("`", espece, "`")  # sécurité pour noms avec espace

  # Filtrer les données
  bdd_plot <- bdd %>%
    filter(!is.na(.data[[espece]])) %>%
    select(Sel, !!sym(espece)) %>%
    rename(Recouvrement = !!sym(espece))

  # Créer le graphique
  p <- ggplot(bdd_plot, aes(x = Sel, y = Recouvrement, fill = Sel)) +
    geom_boxplot(outlier.shape = NA, alpha = 0.7) +
    geom_jitter(width = 0.2, alpha = 0.4, color = "black") +
    labs(
      title = paste("Effet du sel sur le recouvrement de :", espece_label),
      x = "Traitement au sel",
      y = "Recouvrement (%)"
    ) +
  ...
  print(p)  # Affiche le graphique dans la console R
}
...
# Si ton tableau s'appelle déjà `resultats_significatifs`, on le prépare :
resultats_plot <- resultats_significatifs %>%
  arrange(Effet_Mult) %>%
  mutate(
    Espece = factor(Espece, levels = Espece),  # tri dans le plot
    Direction = ifelse(Coef_Sel > 0, "Effet positif", "Effet négatif")
  )

# Graphique
ggplot(resultats_plot, aes(x = Effet_Mult, y = Espece, color = Direction)) +
  geom_point(size = 3) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red") +
  scale_color_manual(values = c("Effet positif" = "forestgreen", "Effet négatif" = "firebrick")) +
  labs(
    title = "Effet du sel sur le recouvrement des espèces (quasi-Poisson)",
    subtitle = "Espèces significatives uniquement (p < 0.05)",
    x = "Effet multiplicatif (exp(Coef_Sel))",
    y = "Espèce",
    color = "Type d'effet"
  ) +
  theme_minimal(base_size = 12) +
geom_text(aes(label = round(Effet_Mult, 5)), hjust = 1.2, color = "black", size = 3)
```
